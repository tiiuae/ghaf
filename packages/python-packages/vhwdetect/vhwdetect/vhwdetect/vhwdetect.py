...
def vmm_args_pci(vmm, vfio_setup, device):
    if vfio_setup and device.driver != "vfio-pci":
        logger.info(f"Setting up VFIO bindings for {device.sys_name}")
        setup_vfio(device)
    ...
    if (
        ...
        for device in pci_devices:
            ...
            if pci_class == 0x03 and "display" in args.devices:
                ...
                pci_args = vmm_args_pci(args.vmm, args.vfio_setup, device)
                if args.vmm == "qemu":
                ...
                vmm_args.append(vmm_args_pci(args.vmm, args.vfio_setup, device))
                ...
                if vendor_id == "8086" and sys_name == "0000:00:1f.3":
                    for dev in pci_devices:
                        if dev.sys_name == "0000:00:1f.0":  # LPC Controller
                            ...
                            vmm_args.append(
                                vmm_args_pci(args.vmm, args.vfio_setup, dev)
                            )
