...
class GpsProcessState():
    def __init__(self):
        ...
        self.stop_wait_asyncio = asyncio.get_event_loop().run_in_executor(None, self.stop_event.wait)
...
async def read_continuous_gps(data):
    process = await asyncio.create_subprocess_exec('gpspipe', '-w', stdin=asyncio.subprocess.PIPE, stdout=asyncio.subprocess.PIPE)
    print(f"GPS reader process PID:", {process.pid}, "starting...")

    while not data.terminate.is_set():
        if process.returncode is not None:
            print(f"gpspipe process has exited")
            break
...
async def wait_connection(gps_state):
    print(f"Websocket listener on localhost:8000.")
    async with websockets.serve(functools.partial(handler,  gps_state=gps_state), "localhost", 8000):
        await gps_state.stop_wait_asyncio
    print(f"Closing websocket listener.")
...
async def main():
    ...
    signal.signal(signal.SIGINT, functools.partial(signal_handler, state_object=gps_state))
    signal.signal(signal.SIGTERM, functools.partial(signal_handler, state_object=gps_state))
