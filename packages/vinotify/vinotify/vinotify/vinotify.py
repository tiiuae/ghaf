        ...
        for event in inotify.read():
            ...
            if directory and event.name:
                ...
                if event.mask & flags.CREATE:
                    # logger.info(f"File create: \"{filepath}\", sending \"{relative_file}\"")
                    # send_path(relative_file, cid, port)
                    logger.info(f'Create: "{filepath}", sending "{relative_dir}"')
                    send_path(relative_dir, cid, port)
                if event.mask & flags.DELETE:
                    logger.info(f'Delete: "{filepath}", sending "{relative_dir}"')
                    send_path(relative_dir, cid, port)
                # if event.mask & flags.MOVED_FROM:
                # logger.info(f"Moved from: \"{filepath}\", sending \"{relative_dir}\"")
                # send_path(relative_dir, cid, port)
                if event.mask & flags.MOVED_TO:
                    logger.info(f'Moved to: "{filepath}", sending "{relative_dir}"')
                    send_path(relative_dir, cid, port)
