...
common_args=(--driverloaded n --nolocking --config "$lvm_config")
...
write_stream_to_image_lv() {
  local name=$1 size_mib=$2 conversion=${3:-notrunc} offset leftover
  ...
  dd of="$image" bs=1M count="$size_mib" seek="$offset" \
    iflag=fullblock oflag=seek_bytes conv="$conversion" status=none
...
for kind in root verity; do
  size_var="${kind}_size_mib"
  file_var="${kind}_file"
  create_image_lv "${kind}_$lv_suffix" "${!size_var}"
  zstd --decompress "$update_dir/${!file_var}" --stdout |
    write_stream_to_image_lv "${kind}_$lv_suffix" "${!size_var}"
done
...
if ((swap_size_mib > 0)); then
  ...
  write_stream_to_image_lv swap "$swap_size_mib" notrunc,sparse <"$work_dir/swap.img"
fi
if ((persist_size_mib > 0)); then
  ...
  write_stream_to_image_lv persist "$persist_size_mib" notrunc,sparse <"$work_dir/persist.img"
fi
