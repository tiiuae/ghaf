...
Populates a disposable build-VM disk with unsigned x86 GPT/FAT/plain LVM.
...
# Reuse the installer marker so first boot encrypts even after a direct flash.
touch "$work/.ghaf-installer-encrypt"
mcopy -i "$esp_image" "$work/.ghaf-installer-encrypt" ::.ghaf-installer-encrypt
mkdir "$work/esp"
