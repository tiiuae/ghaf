...
(qemu_kvm.override {
  ...
  # Sphinx-based doc build fails (ninja step 1837); docs not needed in production
  enableDocs = false;
}).overrideAttrs
  (
    _final: prev: {
      patches =
        ...
        ++ [
          ...
          ./patches/0006-ivshmem-flat-memory-support.patch
          # Increase TPM command timeout
          ./patches/0005-Increase-timeout-in-tpm_util_request.patch
          # USB host autoscan for bus/addr passthrough
          ./patches/0007-usb-host-enable-autoscan-for-bus-addr.patch
        ]
