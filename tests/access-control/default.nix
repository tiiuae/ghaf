...
pkgs.testers.nixosTest {
  ...
  testScript = ''
    ...
    with subtest("agent access control test-1 (PermissionGranted: direct start application request from guivm to appvm)"):
    ...
    with subtest("access control test-2 (get-status to appvm from guivm forbid by admin)"):
        (exit_code, output) = guivm.execute(
    ...
    with subtest("access control test-3 (PermissionDenied: direct GetUnitStatus of givc-hostvm.service on hostvm from guivm)"):
        (exit_code, output) = guivm.execute(
