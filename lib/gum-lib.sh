...
# shellcheck disable=SC2329
show_success() { gum style --foreground="$COLOR_SUCCESS" --bold -- "$@"; }
# shellcheck disable=SC2329
show_error() { gum style --foreground="$COLOR_ERROR" --bold -- "$@"; }
# shellcheck disable=SC2329
show_info() { gum style --foreground="$COLOR_INFO" --margin="$SPACING_INFO_BOTTOM" -- "$@"; }
# shellcheck disable=SC2329
show_warning() { gum style --foreground="$COLOR_WARNING" --bold -- "$@"; }
...
wait_for_user() {
  ...
  echo ""
  gum style --foreground="$COLOR_INFO" --bold "$message"
  read -n 1 -s -r
}
