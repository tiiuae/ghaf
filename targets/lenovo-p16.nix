...
# Generic x86_64 computer -target
{
  ...
  name = "lenovo-p16-gen2";
  system = "x86_64-linux";
  formatModule = nixos-generators.nixosModules.raw-efi;
  lenovo-p16 = variant: extraModules: let
    netvmExtraModules = [
      {
    ...
    guivmExtraModules = [
      {
        ...
        microvm.qemu.extraArgs = [
          ...
          # Trackpoint (red button/joystick)
          "-device"
        ...
        [
          ...
          ({
            ...
            services.udev.extraRules = ''
              ...
              # Laptop touchpad
              SUBSYSTEM=="input",ATTRS{name}=="ELAN0685:00 04F3:320B Mouse",KERNEL=="event*",GROUP="kvm",SYMLINK+="mouse"
              SUBSYSTEM=="input",ATTRS{name}=="ELAN0685:00 04F3:320B Touchpad",KERNEL=="event*",GROUP="kvm",SYMLINK+="touchpad"
              # Laptop TrackPoint
              SUBSYSTEM=="input",ATTRS{name}=="TPPS/2 Synaptics TrackPoint",GROUP="kvm"
              # Lenovo Integrated Webcam
              SUBSYSTEM=="usb",ATTR{idVendor}=="30c9",ATTR{idProduct}=="0050",GROUP="kvm"
            '';
                    ...
                    extraModules = [
                      {
                        ...
                        microvm.qemu.extraArgs = [
                          # Lenovo Integrated USB Webcam
                          "-device"
                          "qemu-xhci"
                          "-device"
                          "usb-host,vendorid=0x30c9,productid=0x0050"
...

          {
            boot.kernelParams = [
              ...
              "iommu=pt"
              # Passthrough Intel WiFi card 8086:7a70
              # Passthrough Intel GPU 8086:4688
              "vfio-pci.ids=8086:7a70,8086:4688"
            ];
  ...
  targets = [
    (lenovo-p16 "debug" debugModules)
    (lenovo-p16 "release" releaseModules)
    (lenovo-p16 "gnome-debug" (gnomeModules ++ debugModules))
    (lenovo-p16 "gnome-release" (gnomeModules ++ releaseModules))
  ];
in {
  nixosConfigurations =
