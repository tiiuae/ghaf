...
in
{
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      ...
      {
        assertion = config.ghaf.boot-health.enable;
        message = "${cfg.target}: secure A/B requires ghaf.boot-health.enable; stock systemd-bless-boot would bless every trial boot.";
      }
    ];
