...
let
  ...
  vmNames =
    cfg.requiredVmNames ++ lib.optional (cfg.debugUnhealthyMicrovm != null) cfg.debugUnhealthyMicrovm;
...
in
{
  options.ghaf.boot-health = {
    ...
    luksMapper = lib.mkOption {
      type = lib.types.str;
      default = "crypted";
      description = "Active outer LUKS mapper required before blessing a generation.";
    };
    ...
    requiredVmNames = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default =
        config.ghaf.common.systemHosts
        ++ lib.optional (config.ghaf.common.adminHost != null) config.ghaf.common.adminHost;
      defaultText = lib.literalExpression ''
        config.ghaf.common.systemHosts ++ optional config.ghaf.common.adminHost
      '';
      description = ''
        Core system and administration MicroVMs that must be active before a
        trial generation is blessed. AppVMs are deliberately excluded.
      '';
    };
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      ...
      {
        assertion = lib.all (name: builtins.hasAttr name config.microvm.vms) cfg.requiredVmNames;
        message = "ghaf.boot-health.requiredVmNames contains a VM that is not configured";
      }
    ];
