...
let
  ...
  # Group devices by their 'vm' attribute
  devicesByVm = builtins.groupBy (
    dev: builtins.elemAt dev.vms 0
  ) config.ghaf.hardware.definition.usb.deviceList;
  ...
  extraRules =
    ...
    in
    lib.strings.concatMapStringsSep "\n" generateRule config.ghaf.hardware.definition.usb.deviceList;
  ...
  config = mkMerge [
    (mkIf
      (
        ...
        && builtins.length config.ghaf.hardware.definition.usb.deviceList > 0
      )
      {
