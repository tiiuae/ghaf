...
# SPDX-License-Identifier: Apache-2.0
{
  ...
  cfg = config.ghaf.hardware.polarfire;
in {
  imports = [
    (modulesPath + "/installer/sd-card/sd-image.nix")
  ];

  options.ghaf.hardware.polarfire.mpfs = {
    enable = lib.mkEnableOption "Make SD card image for PolarFire SoC";
  };

  config = lib.mkIf cfg.mpfs.enable {
    sdImage = {
      compressImage = false;
