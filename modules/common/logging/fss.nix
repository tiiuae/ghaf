  ...
  setupScript = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      generate_fss_key_pair() {
        ...
        if ! sync "$FSS_KEY_FILE"; then
          fss_log fail "Could not flush the new sealing key $FSS_KEY_FILE to durable storage"
          shred -u "$so_tmp" 2>/dev/null || rm -f "$so_tmp"
          return 1
        fi
        ...
        chmod 0400 "$vk_tmp"
        if ! sync "$vk_tmp"; then
          fss_log fail "Could not flush the new verification key to durable storage"
          rm -f "$vk_tmp"
          return 1
        fi
        ...
        if ! durable_write "$VERIFY_KEY_FILE"; then
          fss_log fail "New verification key installed but not durable at $VERIFY_KEY_FILE"
          return 1
        fi
      ...
      retain_previous_verification_key() {
        ...
        if ! durable_write "$retained"; then
          fss_log fail "Could not make the retained FSS verification key $retained durable"
          fss_log fail "Refusing to re-key: putting the outgoing key back rather than risking a torn retain."
          mv -f "$retained" "$VERIFY_KEY_FILE" 2>/dev/null || true
          return 1
        fi
      ...
      record_rekey_history() {
        ...
        # The re-key already happened (caller only gets here after it); the
        # audit trail must not depend on this bookkeeping also succeeding.
        printf '%s\n' \
        ...
        if ! sync "$rh_tmp"; then
          fss_log warn "rekey-history flush failed"
          rm -f "$rh_tmp"
          return
        fi
        ...
        durable_write "$REKEY_HISTORY_FILE" || fss_log warn "rekey-history not durable; mtime fallback orders it"
      }
      ...
      verify_live_sealing_after_activation() {
        ...
        guard_events1="$(fss_clock_step_events)"
        ...
        guard_events2="$(fss_clock_step_events)"
        ...
        if awk -v d="$guard_drift" -v t="$RECOVERY_CLOCK_THRESHOLD_SECONDS" \
          'BEGIN{t=(t<1)?1:t; exit !(d>=t)}' \
          || [ "$guard_events1" != "$guard_events2" ]; then
      ...
      finish_setup() {
        ...
        if [ "$KEY_DURABILITY_FAILED" = 1 ]; then
          fss_log fail "FSS setup finished but the verification key is not confirmed durable"
          exit 1
        fi
        exit 0
      }
      ...
      if ! generate_fss_key_pair; then
        ...
        # The key pair can still be complete (mv succeeded, only the fsync
        # failed): sealing works, so keep activating, but the unit must not
        # report clean success over an unconfirmed-durable key.
        [ -s "$VERIFY_KEY_FILE" ] && KEY_DURABILITY_FAILED=1
      fi
        ...
        if hasStructuredJournald then
          {
            settings.Journal = {
              Storage = "persistent";
              # A bool here renders Seal=true/false; ghaf's own tooling
              # string-matches Seal=yes/no.
