...
let
  ...
  computeScript = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      mod=1
      for ((i=0; i<digits; i++)); do mod=$((mod * 10)); done
      id=$(printf "%0''${digits}d" $((crc % mod)))

      name="''${prefix}-''${id}"

      printf "%s\n" "$name" | tee "$outDir/hostname" "$shareDir/hostname"
      printf "%s\n" "$id" | tee "$outDir/id" "$shareDir/id" > /dev/null
      ...
      printf "%010x\n" "$id" | fold -w2 | paste -sd'-' > "$shareDir/../device-id"
      ...
      ${lib.concatMapStringsSep "\n" (vm: ''
        ...
        echo "$vm_hash" > /persist/storagevm/${vm}/etc/machine-id
      '') activeMicrovms}
    '';
  };
in
