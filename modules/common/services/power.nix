...
let
  ...
  # List of all passthrough PCI devices. The generated list of "vendorId:productId" strings
  # is used to determine suspend actions for PCI devices at runtime
  pciDevices = flatten (
    mapAttrsToList (
      _n: v: (map (device: "${device.vendorId}:${device.productId}") v)
    ) config.ghaf.common.hardware
  );
...
in
{
  options.ghaf.services.power-manager = {
    ...
    vm = {
      ...
      pciSuspend = mkOption {
        type = types.bool;
        default = cfg.vm.fakeSuspend || (cfg.vm.enable && cfg.gui.enable);
        defaultText = literalExpression ''
          config.ghaf.services.power-manager.vm.fakeSuspend || (config.ghaf.services.power-manager.vm.enable && config.ghaf.services.power-manager.gui.enable)
        '';
        description = ''
