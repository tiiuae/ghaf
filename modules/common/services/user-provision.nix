  ...
  config = mkIf cfg.enable {
    ...
    systemd.services = {
      ...
      user-provision-interactive = {
        ...
        unitConfig.ConditionPathExists = "!/var/lib/ghaf/user-provisioning.lock";
        serviceConfig = {
          Type = "oneshot";
          StandardInput = "tty";
          StandardOutput = "tty";
          ...
          ExecStartPost = "${pkgs.coreutils}/bin/install -m 000 /dev/null /var/lib/ghaf/user-provisioning.lock";
          Restart = "on-failure";
          RestartSec = "5s";
        };
      };
      ...
      user-provision-test = mkIf (config.ghaf.profiles.debug.enable && cfg.enableHomed) {
        ...
        serviceConfig = {
          ...
          ExecStartPost = [
            ...
            "${pkgs.coreutils}/bin/install -m 000 /dev/null /var/lib/ghaf/user-provisioning.lock"
          ];
        };
      };
