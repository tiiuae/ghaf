...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    services.endlessh-go = {
      ...
      extraOptions = [ "-interval_ms 10000" ] ++ optionals config.ghaf.profiles.debug.enable [ "-v 1" ];
    };
    ...
    ghaf.firewall = {
      ...
      extra = {
        ...
        input = {
          filter = [
            "-p tcp -d ${cfg.listenAddress} --dport ${toString tarpitListenPort} -j ghaf-fw-conncheck-accept"
          ];
        };
        postrouting = {
          nat = [
