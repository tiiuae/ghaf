...
in
{
  ...
  config = lib.mkIf cfg.enable {
    systemd.services.orbit = {
      ...
      preStart = lib.mkIf (cfg.hostIdentifier == "specified") ''
        uuid_file="/etc/common/ghaf/uuid"
        flagfile="${cfg.rootDir}/osquery.flags"
        if [ -r "$uuid_file" ]; then
          uuid="$(${pkgs.coreutils}/bin/tr -d '\n' < "$uuid_file")"
          if [ -n "$uuid" ]; then
            ${pkgs.coreutils}/bin/mkdir -p "${cfg.rootDir}"
            ${pkgs.coreutils}/bin/printf -- "--specified_identifier=%s\n" "$uuid" > "$flagfile"
          else
    ...
    systemd.user.services."fleet-desktop" = {
      ...
      environment = {
        ...
        PATH = lib.mkForce "${pkgs.xdg-utils}/bin:/etc/profiles/per-user/%u/bin:/run/current-system/sw/bin:/usr/bin:/bin";
      };
