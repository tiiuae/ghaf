...
let
  ...
  winConfig =
    if (lib.hasAttr "reference" config.ghaf) then
      if (lib.hasAttr "programs" config.ghaf.reference) then
    ...
    ghaf.virtualization.microvm.guivm.applications =
      [
...

        {
          ...
          command = "${pkgs.ghaf-audio-control}/bin/GhafAudioControlStandalone --pulseaudio_server=audio-vm:${toString pulseaudioTcpControlPort}";
        }
