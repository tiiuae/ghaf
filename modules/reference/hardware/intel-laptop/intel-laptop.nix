  ...
  host = {
    kernelConfig = {
      kernelParams = [
        ...
        "acpi_osi=linux"
        # Disable kernel modules that are not needed in the host and may conflict with passthrough devices. mei_me, is enabled to reduce delay in starting the ui, though not passed through.
        "module_blacklist=iwlwifi,snd_hda_intel,snd_sof_pci_intel_tgl,snd_sof_pci_intel_mtl,bluetooth,btusb,snd_pcm,xe,snd_sof_pci_intel_lnl,spi_intel_pci,i801_smbus,nouveau,nvidia,nvidiafb,i2c_nvidia_gpu,option,qmi_wwan,cdc_mbim"
      ];
