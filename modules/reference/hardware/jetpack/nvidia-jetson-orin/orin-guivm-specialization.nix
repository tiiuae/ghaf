...
in
{
  ...
  config = {
    ...
    # Patch cosmic-comp for three L4T/Tegra NVIDIA KMS limitations: its EGL does
    # not provide usable EGLDevice enumeration, nvdisplay does not scan out
    # cursor-plane position updates despite accepting them, and implicit
    # DMA-BUF fences can become readable without waking epoll. Use the GBM
    # device node, software-composite the cursor, and poll only outstanding
    # implicit fences with a short-lived timer, respectively.
    nixpkgs.overlays = [
      (_final: prev: {
    ...
    # Tegra implicit DMA-BUF fences can signal without an epoll wake. Enable the
    # compositor's timer-backed readiness check; it retains the surface blocker
    # until every DMA-BUF plane is genuinely readable.
    ...
    # VT-less seatd does NOT disable kernel console input processing (VT-bound
    # seatd does that itself via KDSKBMODE; see seatd/seat.c). tty1 therefore
    # keeps a live line discipline with isig/intr=^C, and greetd + the COSMIC
    # session share tty1's foreground process group -- so a physical Ctrl+C
    # typed into ANY app also reached the kernel console and SIGINT-killed the
    # whole session (silent cosmic-comp death, children "terminated with
    # signal 2", DCE RPC teardown fallout). Mask the console keyboard
    # (KDSKBMODE K_OFF) while greetd runs: input flows exclusively via
    # evdev/libinput; restore UTF-8 mode on stop.
    ...
    # PAM fix for the `cosmic-greeter` service (the screen lock and the
    # cosmic-greeter login UI both use it): its pam_unix ships `use_first_pass`,
    # so it rejects every login without asking for the entered password ("auth
    # could not identify password"). Retain the default `try_first_pass`, but
    # allow pam_unix to prompt when the token is absent.
    #
    # greetd itself needs no equivalent override any more: nixpkgs reduced
    # greetd's stack to `substack login`, so it no longer owns a unix rule, and
    # login-manager.nix handles the login stack. Defining
    # `security.pam.services.greetd.rules.auth.unix.*` here would create a rule
    # with no order and fail evaluation.
    #
    # NOTE: this alone does NOT make the lock screen work -- cosmic-greeter
    ...
    # 2. cosmic-comp used to die on init: Tegra's NVIDIA EGL has no
    ...
    # 3. cosmic-comp runs as the cosmic-greeter user (uid 998), but the L4T GPU
    ...
    # 4. seat0 gets TWO master-of-seat DRM devices -- card0 (nvdisplay, the
    ...
    # 5. No keyboard/mouse in cosmic. vhotplug passes the USB receiver into the
