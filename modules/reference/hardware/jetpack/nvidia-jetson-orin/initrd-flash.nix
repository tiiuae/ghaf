...
let
  ...
  phase2InitrdCommands = lib.optionalString (!cfg.flashScriptOverrides.onlyQSPI) ''
    ...
    # Wait for the host to finish writing, then reboot into the flashed system.
    #
    # The IMAGES_DONE serial message is the fast path, but the flash-initrd's
    # own console shares this same gadget ACM serial, so under mass_storage
    # load the message is unreliable (contention / stdout bleed). Fall back to
    # watching write activity on the target drive: once the host's dd + sync
    # has left the write-sectors counter (field 7 of /sys/block/<dev>/stat)
    # idle for WRITE_IDLE_SECS, the flash is done. Polling can't fire mid-dd
    # because the counter keeps advancing while data is being written.
    #
    # WRITE_IDLE_SECS is the calibration knob: it must exceed the pause between
    # the esp and root dd passes (~2s) with margin. Raise it if a slow host or
    # a stalled USB link ever trips it early.
    WRITE_IDLE_SECS=20
    statfile=/sys/block/$(basename "$LUN_BACKING")/stat
    read_wsec() { set -- $(cat "$statfile" 2>/dev/null) 0 0 0 0 0 0 0; echo "$7"; }
    prev=-1
    idle=0
    waited=0
    while [ $waited -lt 1800 ]; do
      if IFS= read -r -t 5 line <"$ttyGS" 2>/dev/null; then
        echo "  [host] $line"
        case "$line" in
          IMAGES_DONE*) echo "IMAGES_DONE received."; break ;;
        esac
      fi
      cur=$(read_wsec)
      if [ "$cur" != "$prev" ]; then
      ...
      if [ "$cur" -gt 0 ] 2>/dev/null && [ $idle -ge $WRITE_IDLE_SECS ]; then
        echo "Target write activity idle after $cur sectors; assuming flash complete."
        break
      fi
      waited=$((waited + 5))
    done
    ...
    # Best-effort success marker for the host's final wait, then flush and
    # reboot. Reboot is UNCONDITIONAL: the mass_storage LUN holds the target
    # drive open, blocking upstream's normal post-flash reboot, so the device
    # would otherwise hang in the flash initrd regardless of how the wait above
    # exited (IMAGES_DONE, write-idle, or the 30-min cap).
    [ -e "$ttyGS" ] && echo "Flashing platform firmware successful" >$ttyGS
    sync
    sleep 2
    reboot -f 2>/dev/null || echo b >/proc/sysrq-trigger 2>/dev/null || true
  '';
  ...
  phase2HostScript = lib.optionalString (!cfg.flashScriptOverrides.onlyQSPI) ''
    ...
    AVAIL=$(read_message "$SERIAL_PORT" "AVAILABLE_TARGETS=" 60)
    if [ -z "$AVAIL" ]; then
    ...
    read_message "$SERIAL_PORT" "STORAGE_READY" 90
    ...
    # host-side failure.
    read_message "$SERIAL_PORT" "Flashing platform firmware successful" 30 || true
                                    ...
                                    ${
                                      if cfg.flashScriptOverrides.onlyQSPI then
                                        ...
                                        ''
                                          ...
                                          # Wait until a line containing $pat appears (or timeout).
                                          # Prints the text following $pat on stdout, so a "KEY="
                                          # pattern yields the value; a plain substring yields "" and
                                          # is used only for the return code. Returns 1 on timeout or
                                          # a device-reported failure.
                                          read_message() {
                                            local port="$1" pat="$2" timeout="$3"
                                            local end=$((SECONDS + timeout))
                                            echo "Waiting for: $pat (timeout: ''${timeout}s)" >&2
                                            while [ $SECONDS -lt $end ]; do
                                              [ -e "$port" ] || { sleep 1; continue; }
                                              IFS= read -r -t 1 line <"$port" 2>/dev/null || continue
                                              echo "  [device] $line" >&2
                                              case "$line" in
                                                *unsuccessful*) echo "ERROR: Device reported failure" >&2; return 1 ;;
                                                *"$pat"*) printf '%s' "''${line#*"$pat"}"; return 0 ;;
                                              esac
                                            done
                                            echo "ERROR: Timed out waiting for: $pat" >&2
                                            return 1
                                          }
