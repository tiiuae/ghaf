...
let
  ...
  # Explicit display capability (../payload), consumed only for a consistency
  # check below -- disp-vm keeps its own reservedMem/dispCaps/vfioArgs above
  # verbatim (mkPayload doesn't own the dispram_lo_p/dispram_hi_p wrappers).
  inherit (import ../payload { inherit lib pkgs; }) capabilities mkPayload;
  cap = capabilities.dispvm;
  payload = mkPayload cap;

  # dispvm capability must agree with the hand-listed device roles above.
  _capOk =
