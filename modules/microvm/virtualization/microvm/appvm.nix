    ...
    lib.mkIf cfg.enable {
      ...
      systemd.services =
        let
          ...
          # Each AppVM needs its own instance of vsockproxy on the host
          proxyServices = map (
            vm:
            let
              waypipeConfig = config.microvm.vms."${vm.name}-vm".config.config.ghaf.waypipe;
            in
            if waypipeConfig.enable then
              {
                "vsockproxy-${vm.name}" = waypipeConfig.proxyService;
              }
            else
              { }
          ) cfg.vms;
      ...
      # The GUIVM needs to have a dedicated Waypipe instance for each AppVM
          ...
          systemd.user.services =
            let
              waypipeServices = map (
                ...
                if waypipeConfig.enable then
                  {
                    "waypipe-${vm.name}" = waypipeConfig.waypipeService;
                  }
                else
                  { }
              ) cfg.vms;
            in
            lib.foldr lib.recursiveUpdate { } waypipeServices;
        }
      ];
    };
}
