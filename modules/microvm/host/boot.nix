...
let
  ...
  wait-for-user = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      echo "Waiting for ghaf-session to be active..."
      state="inactive"
      until [ "$state" == "active" ]; do
        state=$(systemctl --user is-active ghaf-session.target --machine=${toString config.ghaf.users.loginUser.uid}@.host)
        sleep 1
      done
      echo "ghaf-session is now active"
    '';
  };
in
{
