...
let
  ...
  plymouth-ghaf-background = lib.mkIf cfg.firmwareLogo.enable (
    pkgs.runCommand "plymouth-ghaf-background" { nativeBuildInputs = [ pkgs.imagemagick ]; } ''
      if [ ! -z "${cfg.firmwareLogo.image}" ]; then
  ...
  config = lib.mkIf cfg.enable {
    boot = {
      plymouth = {
        ...
        themePackages = lib.optionals cfg.firmwareLogo.enable [ plymouth-ghaf-background ];
      };
      ...
      ++ lib.optionals cfg.firmwareLogo.enable [ "bgrt_disable=1" ];
      consoleLogLevel = 0;
      initrd.verbose = false;
    };
    systemd.services.plymouth-quit = {
      after = lib.mkIf (cfg.waitForService != null && cfg.waitForService != "") [ cfg.waitForService ];
    };
  };
}
