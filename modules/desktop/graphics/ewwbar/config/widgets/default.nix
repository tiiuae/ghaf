  ...
  ;; Launcher ;;
          ...
          (box
              ...
              (sys_slider
                      ...
                      (revealer 
                              ...
                              (box :orientation "v"
                                (label :text "No audio streams" :visible {arraylength(volume-sink-inputs) == 0} :halign "center" :valign "center")
                                (volume_mixer)
                              )))
              (sys_slider
                      :class "qs-slider"
      ...
      (defwidget volume_mixer []
          (scroll
              :hscroll "false"
              :vscroll "true"
              :height { 50 * min(3,arraylength(volume-sink-inputs)) }
              (box :orientation "v" :space-evenly false
                (for entry in {volume-sink-inputs}
                  (sys_slider 
                      :class "qs-slider"
                      :header {entry.name}
      ...
      (defwidget bar_left []
          (box	
              ...
              :valign "center"
              (launcher)
              (label :text volume-sink-inputs :visible "false")
              (divider)
              (workspaces)))
