...
:::note[Stack boundary]
PR #2199 provides the shared mechanisms. NVIDIA behavior described here is
implemented by follow-up PR #2200, and x86 behavior by follow-up PR #2201;
neither adapter is present in the shared-core diff alone. Until those
follow-ups merge, every flash and initial-install section below documents the
adapter contract, not behavior available from #2199 or `main`.
:::
...
Export a public-only, generation-specific flake input before every candidate
build:

```sh
export GHAF_DEV_KEY_DIR=/secure/local/ghaf-dev-keys
export SECURE_AB_GENERATION=8
...
The exported directory contains only `config.json`, `PK.crt`, `KEK.crt`,
`db.crt`, and `update.pub`. It is safe to import into the Nix store; never point
the flake input at `GHAF_DEV_KEY_DIR` itself. The image records the generation
and all four SHA-256 digests in `/etc/ghaf/update/trust-digests.json`.

Rollback tests may add `--inject-boot-health-failure` when exporting a dedicated
debug build configuration. The shared module rejects that switch on non-debug
images; do not reuse a fault-injection configuration for a normal candidate.

Pass the directory as a pure non-flake input to every image and flash-script
build:
...
For repeatable CI or release builds, make a wrapper flake pin the public build
configuration and set `inputs.ghaf.inputs.secure-ab-build-config.follows` to
that locked input. The same Ghaf revision and lock then resolve to the same
generation and trust root.

When the corresponding follow-up adapter is applied, its flash path requires
the runtime `GHAF_DEV_KEY_DIR` to contain the same public files and verifies
that `db.key` matches `db.crt` and `update.key` matches `update.pub`. A
public-trust mismatch fails exactly with:
...
The repository default `secure-ab-build-config` explicitly selects CI trust and
generation 1. It uses repository development UEFI certificates and the public
RFC 8032 test-vector key. The corresponding private keys are public knowledge,
so the module permits this input only on debug images and adapters must refuse
to deploy it. Evaluation prints exactly:

```text
<target>: using repository CI-only trust from secure-ab-build-config; this image is restricted to debug build coverage and must not be deployed.
```

An external pure build configuration prints exactly:

```text
<target>: using pure external public trust from secure-ab-build-config; private signing keys remain outside the Nix store.
```

An invalid schema, non-positive generation, or missing public file fails pure
evaluation. Secure A/B flashing and update signing additionally require the
external private keys at runtime and fail before mutation if they are absent.
...
original fail-closed check as defense in depth when invoked directly. This
flash behavior is implemented by the NVIDIA adapter PR, not by the shared-core
PR alone.
...
Secure A/B targets also declare fixed root and verity slot capacities. The
initial image build fails when an unpacked artifact exceeds its declared
capacity. Slot capacity is therefore a platform policy and does not change with
the size of the release used to install a device.
...
ghaf-fetch-update "$UPDATE_HTTP_ORIGIN" "$SECURE_AB_GENERATION"
```
...
Run the dry-run plan before mutation. A candidate whose unpacked root or verity
artifact exceeds the platform's fixed slot capacity is incompatible and must
fail before writes; do not depend on unreserved VG space to grow it. The active
pair is never shrunk or overwritten. Keep room on the ESP for the temporary UKI
plus the two managed entries. Prune OCI
...
6. Resize the LVM PV.
7. Create the empty root and verity LVs at the platform's fixed slot capacities.
8. Create the randomly encrypted 4096 MiB swap LV.
9. Allocate the remaining space to the Btrfs persist LV and format it.
...
timeout 2h nix build \
  --override-input secure-ab-build-config "path:$SECURE_AB_BUILD_CONFIG" \
  .#intel-laptop-debug-secure-ab -o result-x86-initial
timeout 2h nix build \
  --override-input secure-ab-build-config "path:$SECURE_AB_BUILD_CONFIG" \
...
| Persist, swap, empty-slot provisioning | `modules/partitioning/firstboot-persist.nix` |
