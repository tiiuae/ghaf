...
positive integer. `system` is the Nix platform identifier for the image,
`build-system` is the platform that built it, and `target` must exactly equal
the target configured on the device.

Sign an unsigned build output without modifying its source directory:

```sh
nix run .#ghaf-mk-artifacts -- \
  --input-manifest result-update/ghaf_<version>_<hash>.manifest \
...
destination. The `ghaf-fetch-update` script rejects path-like manifest names
and publishes each file only after its download succeeds:

```sh
ghaf-fetch-update 'http://<developer-interface-address>:<port>' '<candidate-generation>'
```
...
| Update signer | `packages/pkgs-by-name/ghaf-mk-artifacts/` |
