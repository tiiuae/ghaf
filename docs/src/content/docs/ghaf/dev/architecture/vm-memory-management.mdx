...
Ghaf addresses this with a layered defense against OOM crashes: zram compressed swap inside every VM, virtio-balloon deflation for QEMU App VMs, and host-level disk swap that absorbs the resulting pressure.
...
## Layer 2: Balloon deflateOnOOM (QEMU App VMs)

QEMU App VMs use the [virtio-balloon](https://blog.pmhahn.de/virtio-balloon/) device for dynamic memory management. The host memory manager can inflate the balloon (reclaiming guest pages for the host) or the guest can deflate it (reclaiming pages back from the host).
...
crosvm App VMs also do not use them because `ghaf-mem-manager` currently
controls balloons through QEMU QMP. A crosvm App VM receives its configured
`mem` allocation directly and relies on zram and host swap for memory pressure.

## Layer 3: Host Disk Swap

When QEMU App VM balloons deflate, the QEMU process on the host consumes more physical memory. The host has a 12 GB disk swap partition that absorbs this pressure. The host kernel can page out its own processes or inflate balloons on other idle QEMU App VMs (via `ghaf-mem-manager`) to rebalance. crosvm App VMs use the same host swap as a pressure backstop but do not participate in this QMP-based rebalancing.
...
4. **Balloon deflates if needed**: In a QEMU App VM, the balloon shrinks and the guest reclaims pages from the host. crosvm App VMs skip this layer.
...
| Option | Description |
|--------|-------------|
| `balloonRatio` | Multiplier for QEMU memory allocation beyond base `mem`. A ratio of 2 means QEMU allocates `mem * (balloonRatio + 1)`. Ignored for crosvm App VMs. |
