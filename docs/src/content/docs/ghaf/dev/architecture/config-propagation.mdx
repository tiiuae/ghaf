...
| Users | `profile`, `admin`, `managed` | May vary by VM role |
| Hardware | `kernel`, `passthrough` | Device-specific |
| GIVC | `cliArgs`, `appliances` | VM-specific IPC config |
...
hostConfig = {
  ...
  users = {
    ...
    admin = { /* admin user config */ };
    managed = { /* managed users config */ };
  };

  # Common settings namespace
  common = { /* common config from host */ };

  # GIVC configuration
  givc = {
    cliArgs = "--addr ...";
    appliances = [ /* list of services */ ];
  };

  # Hardware configuration
  hardware = {
    passthrough = { /* passthrough devices */ };
  };
};
```
