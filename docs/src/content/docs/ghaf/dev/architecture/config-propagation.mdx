...
| Development | `debug.tools.enable`, `nix-setup.enable` | Consistent dev experience |
...
| Security | `security.audit.enable`, `security.ssh.debug`, `security.ssh.release` | Consistent security posture |
...
}:
{
  config = {
    ...
    ghaf.security.ssh.debug.enable = globalConfig.security.ssh.debug.enable;
    ghaf.development.debug.tools.enable = globalConfig.development.debug.tools.enable;
...
{ configHost }:
{
  ...
  ghaf.security.ssh.debug.enable =
    configHost.ghaf.security.ssh.debug.enable;
}
```
