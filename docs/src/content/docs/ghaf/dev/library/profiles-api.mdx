...
| `security.ssh.debug.enable` | true | false | false |
...
```nix
{
  ...
  # SSH — security posture on every machine. debug and release are mutually exclusive.
  # See the SSH Access reference (/ghaf/dev/ref/ssh) for the full mechanism and hardening.
  security.ssh = {
    debug.enable = bool;               # Default: false  (unprotected: password + dev keys)
    release = {                        # hardened, certificate/key-only sshd
      enable = bool;                   # Default: false
      authorizedKeys = [ str ];        # Default: []      (hardware-backed / YubiKey recommended)
      trustedUserCAKeys = [ str ];     # Default: []      (SSH user-CA public keys)
      allowedPrincipals = [ str ];     # Default: []      (empty => module default: admin user)
      authorizedKeysOptions = str;     # Default: "restrict,pty,port-forwarding,verify-required"
    };
  };
