...
| `logging.enable` | true | true | false |
...
```nix
{
  ...
  security.ssh = {
    ...
    release.enable = bool;             # Default: false  (hardened, certificate/key-only sshd)
    # Posture only. The key material is deployment data and is written through
    # ghaf.org.identity.ssh (see /ghaf/dev/org-config).
  };
  ...
  logging = {
    ...
    server.enable = bool;      # Default: true (endpoint comes from ghaf.org)
  };
