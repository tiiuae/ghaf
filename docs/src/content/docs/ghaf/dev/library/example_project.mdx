...
1. Create a template project as described in the [Ghaf as Library](/ghaf/dev/library) section.
...
1. Add a Wi-Fi network name and password to the _lenovo-x1-carbon.nix_ file instead of `#networks."ssid".psk = "psk"`.
2. Build and run the image. For more information, see [Running Ghaf Image for Lenovo X1](/ghaf/dev/ref/build_and_run#running-ghaf-image-for-lenovo-x1).
...
3. When the necessary device names are found, add them to `services.udev.extraRules` in the _lenovo-x1-carbon.nix_ file, rebuild the image and test the changes.
