...
Whether to log is a platform decision and lives here; *where* the logs go is deployment data
and lives in [`ghaf.org.telemetry.logging`](/ghaf/dev/org-config/).
...
Posture owns the switches; the key material itself is deployment data and lives in
[`ghaf.org.identity.ssh`](/ghaf/dev/org-config/).
...
### Values that moved to `ghaf.org`

These six were global config until the organization layer landed. They are deployment data,
not platform posture, and carrying them in both places gave one value two entry points with
different merge behaviour. They are now set through [`ghaf.org`](/ghaf/dev/org-config/), which
reaches the host and every VM:

| Removed from `ghaf.global-config` | Now set through |
| --- | --- |
| `logging.server.endpoint` | `ghaf.org.telemetry.logging.endpoint` |
| `logging.logseald.revokedPeerKeys` | `ghaf.org.telemetry.logging.logseald.revokedPeerKeys` |
| `security.ssh.release.authorizedKeys` | `ghaf.org.identity.ssh.releaseKeys` |
| `security.ssh.release.trustedUserCAKeys` | `ghaf.org.identity.ssh.trustedUserCAKeys` |
| `security.ssh.release.allowedPrincipals` | `ghaf.org.identity.ssh.allowedPrincipals` |
| `security.ssh.release.authorizedKeysOptions` | `ghaf.org.identity.ssh.authorizedKeysOptions` |

Setting one of the old paths is a hard evaluation error (`The option … does not exist`). That
is deliberate: a silent fallback on key material is worse than a build that stops.
