...
profile overrides NetVM, AudioVM, and GUIVM to crosvm; aarch64 and generic VM
targets retain QEMU. AdminVM uses crosvm when storage encryption is disabled.
Encrypted AdminVMs remain on QEMU because the crosvm runner does not yet
provide the TPM path used to seal their storage keys. Each system VM can
override the profile default independently:
