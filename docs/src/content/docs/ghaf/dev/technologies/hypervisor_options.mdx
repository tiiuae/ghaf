...
store-disk variants. The accelerated AGX and NX GUI-VM targets and their
generated variants also select crosvm for all system VMs. Other
machine-specific x86, aarch64, and generic VM targets keep their existing
selections. Generic Intel laptop targets also select crosvm for encrypted
AdminVM variants and use crosvm's TPM passthrough path. Each system VM can
...
AGX and NX GUI-VM targets select crosvm for their App VMs; other
machine-specific x86, aarch64, and generic VM targets retain QEMU. Generated
accelerated Orin variants inherit this selection. The target default and
