...
Unencrypted App VMs use crosvm by default on both x86 and NVIDIA platforms.
The default and individual App VMs can be overridden through the same
...
`chromium`. If storage encryption is enabled in the global VM profile, an App
VM with no explicit `vmm` selection automatically retains QEMU. An explicit
crosvm selection fails evaluation in that configuration because the pinned
crosvm does not implement Ghaf's swtpm backend.

crosvm App VMs also run without virtio ballooning. Ghaf's current memory
manager controls balloons through QEMU QMP, so crosvm receives the App VM's
configured `mem` allocation directly instead of the QEMU
`mem * (balloonRatio + 1)` allocation. The guest still uses zram swap.
...
supported by crosvm; runtime PCI attachment remains QEMU-only.
