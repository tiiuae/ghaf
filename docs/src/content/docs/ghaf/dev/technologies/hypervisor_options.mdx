...
store-disk variants. The accelerated AGX GUI-VM target and its generated
variants also select crosvm for all system VMs. Other machine-specific x86,
aarch64, and generic VM targets keep their existing selections. Generic Intel
laptop targets also select crosvm for encrypted AdminVM variants and use
crosvm's TPM passthrough path. Each system VM can override the target default
independently:
...
App VMs use QEMU by default. Generic `intel-laptop` targets and the accelerated
AGX GUI-VM target select crosvm for their App VMs; other machine-specific x86,
aarch64, and generic VM targets retain QEMU. Generated accelerated AGX variants
inherit this selection. The target default and individual App VMs can be
overridden through the same `ghaf.virtualization.vmConfig` interface:
