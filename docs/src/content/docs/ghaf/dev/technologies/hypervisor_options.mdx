...
store-disk variants. Every exported NVIDIA Jetson Orin AGX, AGX64, Industrial,
NX, and verity target also selects crosvm for all system VMs. Other
machine-specific and generic VM targets keep their existing selections.
Generic Intel laptop targets also select crosvm for encrypted AdminVM variants
and use crosvm's TPM passthrough path. Each system VM can override the target
...
App VMs use QEMU by default. Generic `intel-laptop` targets and every exported
NVIDIA Jetson Orin target select crosvm for their App VMs; other
machine-specific and generic VM targets retain QEMU. The target default and
individual App VMs can be overridden through the same
...
use the same configured CID. Runtime USB attachment through the device-manager
compatibility interface is supported by crosvm. The manager records the crosvm
guest port for each host USB device so identical devices can be detached
independently and restored safely after daemon restarts. x86 laptop system VMs
also resolve configured PCI devices at startup and support runtime PCI
attachment with crosvm's removable VFIO slots.
