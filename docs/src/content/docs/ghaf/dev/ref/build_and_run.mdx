...
<Aside type="note">
  **Build Time Warning**: All Ghaf builds take 45+ minutes, with some requiring 2-3 hours. Plan accordingly and ensure you have a stable internet connection and sufficient disk space (minimum 20GB free).
</Aside>
...
- **Architecture**: x86_64 (for building all targets)
...
4. **For ARM/AArch64 targets**: Set up a remote builder for cross-compilation:
...
1. **Build the image** (takes 60-120 minutes):
   ```sh
...
### NVIDIA Jetson AGX Orin (Primary ARM64 target)

This is the primary reference device for Ghaf on AArch64 architecture.

1. **Build flash script** (takes 90-180 minutes):
   ```sh
   # For 32GB AGX version:
   nix build .#nvidia-jetson-orin-agx-debug-from-x86_64-flash-script

   # For 64GB AGX version:
...
1. **Run VM directly** (takes 45-90 minutes to build):
   ```sh
