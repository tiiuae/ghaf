...
### Intel laptops (Recommended for x86_64)

One generic image covers every Intel laptop in the fleet — X1 Carbon, Dell Latitude Rugged, System76 Darter Pro and others. See [Hardware Configuration](/ghaf/dev/ref/hw-config/) for the feature axes and for the boards that still need their own target.
...
2. **Prepare USB boot media**:
   ```sh
   sudo nix develop --command ghaf-flash -d /dev/<YOUR_USB_DRIVE> -i result/ghaf-image.raw.zst
   ```

   ⚠️ **Warning**: Replace `<YOUR_USB_DRIVE>` with your actual USB device (e.g., `/dev/sdb`). This will erase all data on the USB drive.

3. **Boot from USB**: Insert the USB drive and boot the laptop from the USB device.
...
## Running Ghaf on an x86 Intel Laptop

For general x86_64 hardware, without the laptop passthrough stack:
...
2. **Flash to USB**:
   ```sh
   ./packages/pkgs-by-name/flash-script/flash.sh -d /dev/<YOUR_USB_DRIVE> -i result/ghaf-image.raw.zst
   ```
...
## Running a Ghaf Image on an Intel Laptop

`intel-laptop-debug` is the reference x86_64 image for the Ghaf project.

Do the following:

1. To build the target image, run the command:
   ```sh
   nix build github:tiiuae/ghaf#intel-laptop-debug
   ```
2. After the build is completed, prepare a USB boot media with the target image you built using `ghaf-flash`:
   ```sh
   sudo nix develop --command ghaf-flash -d /dev/<YOUR_USB_DRIVE> -i result/ghaf-image.raw.zst
   ```
3. Boot the computer from the USB media.
