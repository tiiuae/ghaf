...
**Location**: `modules/reference/hardware/intel-laptop/`
**Purpose**: The generic image for every Intel laptop in the fleet
**Features**:
- PCI passthrough resolved at runtime by device class, not per-board ID
- Fingerprint reader, webcam and hotkey passthrough matched from union quirk lists
- Suspend mode selected at boot from a DMI model list
...
GUI VM D-Bus proxies are declared through `ghaf.givc.guivm.dbusProxies`.
Each entry creates `systemd.services.dbus-proxy-<name>` when GIVC and its GUI VM module are enabled:

```nix
ghaf.givc.guivm.dbusProxies.networkmanager = {
  description = "DBus proxy for Network Manager gui-vm";
...
The shared module owns startup ordering, socket readiness, and proxy invocation.
Feature modules retain their enablement conditions and D-Bus access policies.
The socket must already be configured in the GIVC socket transport.
Apply service-specific hardening or restart overrides through `systemd.services.dbus-proxy-<name>`.

#### profiles-laptop-x86
**Location**: `modules/profiles/laptop-x86.nix` (was `profiles-workstation`, still available as an alias)
...
```nix
{
  imports = [
    ...
    inputs.ghaf.nixosModules.profiles-laptop-x86
  ];
