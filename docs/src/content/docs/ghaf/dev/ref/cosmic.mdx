...
<Aside>
  The two layers are complementary rather than redundant, and a locked-down
  product usually wants both: `disabledShortcuts` covers the bindings shipped
  today, while `systemActions` also covers a keyboard with a dedicated key, or a
  binding a future COSMIC adds. Neither is a single "lock down the desktop"
  switch, deliberately — disabling `Close` strands a user who opened a window
  with no way to shut it, so that call belongs to the product, per binding.
</Aside>
