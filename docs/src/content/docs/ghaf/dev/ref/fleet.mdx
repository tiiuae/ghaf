...
The orbit service includes a `ConditionPathExists` directive to wait for the hostname file and, when `enrollSecretPath` is used, to enroll the secret file before starting.
