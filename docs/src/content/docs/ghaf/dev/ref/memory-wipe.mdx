...
This feature is **enabled by default** for all x86_64 platforms through the `hardware-x86_64-generic` module.

To disable it on x86_64 platforms:

```nix
{
  ghaf.host.memory-wipe.enable = false;
}
```
...
- **Module**: `modules/host/memory-wipe.nix`
- **Imported by**: `modules/common/flake-module.nix`
- **Default enablement**: `modules/hardware/x86_64-generic/x86_64-linux.nix` (for x86_64 platforms)
