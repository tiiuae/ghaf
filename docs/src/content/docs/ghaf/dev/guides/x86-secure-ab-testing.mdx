...
volume key through the existing deferred-encryption flow, then enrolls TPM2/FIDO2
and recovery when available. Release uses an interactive, nonempty passphrase;
debug retains the existing automated development policy. No empty bootstrap
...
Per laptop, require Secure Boot, distinct per-device volume keys, recovery unlock,
