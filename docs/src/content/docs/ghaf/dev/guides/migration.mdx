  ...
  outputs = { ghaf, ... }:
    ...
    in
    {
      nixosConfigurations.my-product-debug = mkGhafConfiguration {
        name = "my-product";
        hwDefinition = ghaf.nixosModules.hardware-lenovo-x1-carbon-gen11;
        profile = ghaf.nixosModules.profiles-laptop-x86;
        globalConfigOverrides = ghaf.lib.ghaf.profiles.debug;
        extraModules = [
          ({ config, ... }:
          let
            baseVm = config.ghaf.profiles.laptop-x86.guivmBase;
