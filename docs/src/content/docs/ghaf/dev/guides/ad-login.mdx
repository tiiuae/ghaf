...
| `ad_gpo_map_interactive` | Map Ghaf display manager, login, and su services to the Windows "interactive logon" right |
| `ad_gpo_map_remote_interactive` | Map SSH PAM service to the Windows "remote interactive logon" right |

### PAM Strict Access

The SSSD service module (`modules/common/services/sssd.nix`) enables `sssdStrictAccess` on the interactive PAM entrypoints Ghaf exposes, including the display manager, `su`/`su-l`, and the standard `login`/`sshd` stacks. This ensures that SSSD account-phase deny decisions (from GPO evaluation) are enforced consistently for graphical, TTY, switch-user, and SSH logins.

### Sudo Privileges for AD Groups

To grant sudo privileges to specific Active Directory administrative groups, add the group rule to `security.sudo.extraConfig`:

```nix
security.sudo.extraConfig = ''
  %<ad-group-name> ALL=(ALL) ALL
  %<ad-group-name>@<domain> ALL=(ALL) ALL
'';
```
...
3. SSSD starts automatically upon domain enrollment (triggered by `systemd.paths.sssd-keytab` when `/etc/krb5.keytab` is created). If needed, you can also restart it manually:
   ```bash
