...
manifests=("result-$ORIN-ghafImage"/*.manifest)
...
a complete `+3.efi` trial and candidate-specific one-shot selection. The
persistent `LoaderEntryDefault` must remain unchanged; compare its efivarfs bytes
before/after because `bootctl list` may display the one-shot as the next default.
Installation must not reboot. Reboot manually and observe the health/rollback cases below.
