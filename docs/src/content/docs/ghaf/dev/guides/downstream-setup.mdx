...
│   │   └── my-device.nix
│   └── my-org.nix            # Your organization's values (ghaf.org)
├── targets/
│   └── my-product/
...
in
{
  flake = let
    ...
    my-product-debug = mkGhafConfiguration {
      ...
      # Additional modules
      extraModules = [
        # Your organization's own values -- logging endpoint, SSH rosters,
        # Secure Boot keys, admin account. See Organization Configuration
        # (/ghaf/dev/org-config). Ghaf's images enable the TII reference org
        # module instead; a downstream supplies its own here.
        inputs.self.nixosModules.my-org
