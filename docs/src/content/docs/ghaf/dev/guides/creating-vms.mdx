...
{ config, lib, pkgs, globalConfig, hostConfig, inputs, ... }:
{
  ...
  ghaf = {
    ...
    development.debug.tools.enable = globalConfig.development.debug.tools.enable;
    security.ssh.debug.enable = globalConfig.security.ssh.debug.enable;
    givc = {
...
App VMs get vTPM support automatically when storage encryption is enabled globally — no per-VM configuration is needed. When encryption is off, you can still opt in to vTPM by setting `vtpm.enable`:
