---
title: Running Android Cuttlefish Virtual Device on Ghaf
---
...
1. Download _host_package_ (includes binaries and scripts that must be run on the host machine to set up and run the Cuttlefish virtual device) and _aosp_cf_phone-img_ (a system image) files from the Android CI server and copy them to Ghaf:
