...
Forward Secure Sealing (FSS) is an optional systemd journal mechanism that
provides cryptographic tamper evidence for Ghaf audit logs. It uses HMAC-SHA256
chains to seal journal entries at regular intervals, making modification of
sealed entries detectable.

Ghaf keeps FSS available, but disables it globally by default. The default
tamper-evidence mechanism is the clock-independent [logseald
service](/ghaf/scs/logseald/). The two implementations are independent and can
be enabled or disabled separately without disabling journald or log forwarding.
...
Enable FSS across the host and every logging-enabled VM with the global policy:

```nix
ghaf.global-config.logging.fss.enable = true;
```

When the global policy is enabled, the component-level option defaults to the
value of `ghaf.logging.enable`. The component options are:
...
To disable FSS globally while keeping the logging pipeline and logseald
unchanged, use:

```nix
ghaf.global-config.logging.fss.enable = false;
```

This explicitly configures journald with `Seal=no` and removes the Ghaf FSS
setup, verification, and activation dependencies. Clock-jump recovery keeps
log forwarders healthy, but skips FSS-specific journal restarts, rotations, and
recovery metadata while FSS is disabled. Existing journals, sealing keys,
verification keys, and metadata are retained so FSS can be re-enabled later by
setting the option to `true`. Reboot after changing the option so every VM
