...
Two scripts are available for deployed systems:

- `fss-test`: quick health check and policy classification
- `fss-debug`: full evidence packet for root-cause analysis
- `fss-rootcause`: labeled checkpoint workflow and packet comparison for intermittent issues

```bash
# Build the scripts
nix build .#checks.x86_64-linux.fss-test
nix build .#checks.x86_64-linux.fss-debug
nix build .#checks.x86_64-linux.fss-rootcause

# Deploy and run the quick health check
scp result/bin/fss-test root@ghaf-host:/tmp/
ssh root@ghaf-host /tmp/fss-test

# Deploy and run the deep debug tool
scp result/bin/fss-debug root@ghaf-host:/tmp/
ssh root@ghaf-host sudo /tmp/fss-debug

# Deploy and run the root-cause workflow helper
scp result/bin/fss-rootcause root@ghaf-host:/tmp/
ssh root@ghaf-host sudo /tmp/fss-rootcause help
```
...
`fss-debug` writes a timestamped packet under `/var/tmp/fss-debug-<UTC>/` by default and captures:

1. System identity (`hostname`, `machine-id`, shared Ghaf identity files, `device-id`)
2. FSS runtime layout (`fss-config`, sealing key path, verification key path, active `system.journal`)
3. Journal directory inventory, mounts, and file metadata
4. Relevant unit state and recent logs for FSS, journald, identity, and recovery services
5. Current journald configuration and audit rules
6. Full `journalctl --verify` output plus per-file isolates for failed journals
7. A summary classifying the result with the same policy as `journal-fss-verify.service`

`fss-rootcause` wraps repeated `fss-debug` packets into a reproducible investigation session. Use it when failures are intermittent or seem correlated with idle/suspend transitions:
...
On the host, collect the matching microvm and power-management context:
...
For a full local evidence packet instead of ad-hoc command collection:

```bash
sudo fss-debug
```

For intermittent `user-*.journal`, `system@...journal`, or suspend/resume-correlated corruption, use the checkpoint workflow instead of one-off commands:

```bash
sudo fss-rootcause help
```
