...
![Supply Chain Attacks Diagram](@assets/ghaf/threat_processing.drawio.png "Supply Chain Attacks")
...
The software artifact should be encrypted on each possible transition phase and its integrity should be verified at each destination. Each build should be accompanied by means of [_software bill of materials (SBOM)_](/ghaf/appendices/glossary#sbom), identifying all the components that the software package consists of.
