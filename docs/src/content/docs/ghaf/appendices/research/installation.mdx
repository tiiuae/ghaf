---
title: Approaches to Ghaf System Installation
---
...
For information on how to build and run a Ghaf image, see [Build & Run](/ghaf/dev/ref/build_and_run) for details.
...
Ghaf introduced a modular structure for an [interactive installer](/ghaf/dev/ref/installer). The initial Ghaf reference installer still uses a raw disk image per target device. In practice, it just writes the raw disk image to the target device's internal persistent memory, for example, NVMe.
