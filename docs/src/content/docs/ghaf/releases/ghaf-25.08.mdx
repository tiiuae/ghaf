...
Fixed bugs that were presentin the ghaf-25.06 release:

    * Lenovo X1: Suspend -power state is not working
    * Lenovo X1: Some apps may not open at first boot after installation
    * Duplicated settings between Ghaf Control Panel and Cosmic Settings
    * Yubikey for unlocking does not workaround
