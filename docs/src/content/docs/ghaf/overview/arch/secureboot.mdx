...
Jetson Orin targets expose two build artifacts:

1. The flash script (`nix build .#nvidia-jetson-orin-agx-debug-from-x86_64-flash-script`) which orchestrates NVIDIA's flashing tools and includes its matching default image.
...
For non-verity targets, `-s` is optional: omitting it flashes the matching image included with
`flash-ghaf-host`. An explicit `-s` overrides that image with the externally signed image
and selects Secure Boot enrollment firmware.
Fixed partition sizing keeps the underlying firmware flasher independent of the image;
building `*-flash-script` also builds or downloads its default image.
