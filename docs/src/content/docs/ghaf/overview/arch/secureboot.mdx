...
Whether `-s` is optional depends on how the board sizes its partitions. Every non-verity Orin board pins
`ghaf.hardware.nvidia.orin.flashScriptOverrides.appPartitionSizeBytes`, so `flash.xml` carries static ESP/APP sizes and the
flash script embeds no image at all. For those targets `-s` is **required**, and running without it exits with
`ERROR: this flash script was built without an embedded sdImage`. This is deliberate: deriving the sizes from the built
image instead puts `system.build.sdImage` -- and through it every microvm -- into the flash script's *evaluation* closure,
which costs roughly 10 GiB of RAM to evaluate and exceeds the CI eval worker's memory limit.

The verity targets use a different partition template that always embeds the image, so for those `-s` stays optional and
omitting it flashes the bundled image.
...
To make QSPI-side enrollment selectable at flash time without requiring an evaluation-time `signedArtifactsPath`, the `*-flash-script` output ships **two** pre-built QSPI firmware variants behind a single `flash-ghaf-host` entrypoint: one without enrollment material (the default) and one with the `UefiDefaultSecurityKeys` DTBO plus PK/KEK/db ESLs baked in (selected by `--secure-boot`). `-s/--signed-sd-image` is deliberately **not** the selector: with static APP sizing every flash supplies an image with `-s`, signed or not, so keying Secure Boot off it would brick every unsigned flash. The wrapper dispatches on its own `--secure-boot` flag:

```sh
nix build .#nvidia-jetson-orin-agx-debug-from-x86_64-flash-script
SIGNED_SD_IMAGE=$(nix path-info .#nvidia-jetson-orin-agx-debug-from-x86_64)

# Unsigned QSPI, Secure Boot stays disabled. -s is still required: the script embeds no image.
sudo ./result/bin/flash-ghaf-host -s "$SIGNED_SD_IMAGE"

# SB-enabled QSPI: signed BOOTAA64.EFI swap + PK/KEK/db enrolled on first boot.
# Pair --secure-boot with a *signed* image, or the enrolled firmware refuses the unsigned BOOTAA64.EFI.
sudo ./result/bin/flash-ghaf-host --secure-boot -s "$SIGNED_SD_IMAGE"
```
...
the x86 laptop flow and from the sd-image flow described above. The NVIDIA UEFI firmware supports standard UEFI Secure Boot
