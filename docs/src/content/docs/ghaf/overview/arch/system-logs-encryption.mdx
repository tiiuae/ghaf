...
When logging and `ghaf.logging.recovery.enable` are enabled,
`ghaf-clock-jump-watcher.service` detects changes in wall-clock time.
`ghaf-journal-alloy-recover.service` restarts installed log forwarders with an
uptime-based cooldown. It does not restart journald or Fail2Ban, rotate journals,
or wait for clock synchronization. Logseald seals independently of this recovery
path. Journald's built-in sealing is explicitly disabled with `Seal=no`.
