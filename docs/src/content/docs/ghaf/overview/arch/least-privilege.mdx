...
- Privileged features are opt‑in via profiles
  - Debug/administrative access is gated by explicit profiles rather than being available by default:
    - modules/microvm/sysvms/netvm.nix derives several toggles from ghaf.profiles.debug.enable (for example ghaf.systemd.withDebug, security.ssh.debug, development.debug.tools). In production profiles, these remain disabled, preventing unnecessary privileges and open endpoints.
