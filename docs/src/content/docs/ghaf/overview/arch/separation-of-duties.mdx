...
- Separation through profiles and build-time gating
  ...
  - Evidence in code: in modules/microvm/sysvms/netvm.nix, multiple toggles derive from ghaf.profiles.debug.enable:
    - ghaf.systemd.withDebug, development.ssh.daemon.enable, development.debug.tools.enable, nix-setup.enable are all wired to the debug profile. This prevents routine operators or workloads from gaining access to debug interfaces unless an authorized maintainer explicitly enables the profile.
