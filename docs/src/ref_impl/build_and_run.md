...
2. To flash the Ghaf SD image:
   1. Prepare the SD card with the target HW image you built: `dd if=./result/nixos.img of=/dev/<YOUR_SD_DEVICE> bs=32M`.
     - Insert an SD card into the board and switch the power on.

   2. You can directly flash nixos image to onboard MMC card: `dd if=./result/nixos.img of=/dev/<YOUR_MMC_DEVICE> bs=32M`.
