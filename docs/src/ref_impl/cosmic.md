...
COSMIC is the default desktop environment in Ghaf. It is added from an external Nix flake, [nixos-cosmic](https://github.com/lilyinstarlight/nixos-cosmic), as it is not yet available in the official Nixpkgs repository.

If you prefer, you can switch to `labwc` by updating the graphics configuration as follows:

```nix
profiles.graphics.compositor = "labwc";
```

This will replace COSMIC with `labwc` as the active compositor.
...
```
modules/desktop/graphics/cosmic-config/
├── cosmic-config-to-yaml.sh    # Helper script to convert `.config/cosmic` to a YAML config
└── cosmic.config.yaml          # Main COSMIC desktop configuration
